...
def load_bvh_jointselector(bvhfile):
    ...
    mexp_full = Pipeline([
        ('jtsel',
         JointSelector(["b_root", "b_spine0", "b_spine1", "b_spine2", "b_spine3", "b_neck0", "b_head", "b_r_shoulder",
...
def prepare_h5_unclipped_test(dataroot, h5file, participant, word2vector):
    metadata_path = os.path.join(dataroot, "metadata.csv")
    # dataroot, f"{dataset_type}_interloctr_v0.h5", "interloctr", word2vector
    num_speakers, metadict_byfname, metadict_byindex = load_metadata(metadata_path, participant)
    filenames = sorted(metadict_byfname.keys())
...
if __name__ == "__main__":
    ...
    dataset_type = "tst"
    dataroot = os.path.join(args.dataset_path, dataset_type)
    prepare_h5_unclipped_test(dataroot, f"{args.output_path}/{dataset_type}_main-agent_v0.h5", "main-agent",
                              word2vector)
    prepare_h5_unclipped_test(dataroot, f"{args.output_path}/{dataset_type}_interloctr_v0.h5", "interloctr",
                              word2vector)
